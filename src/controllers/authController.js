...
exports.loginPost = async (req, res) => {
  try {
    ...
    req.session.user = { id: u.id, nome: u.nome, email: u.email, cpf: u.cpf };
...
exports.registerPost = async (req, res) => {
  try {
    const { nome, email, senha, cpf, next } = req.body;
    if (!nome || !email || !senha || !cpf) return res.status(400).render('login', { pageTitle: 'Entrar', next: next || '/carrinho', error: 'Preencha todos os campos' });
    ...
    const id = await createUser({ nome, email: emailNorm, senha_hash: hash, cpf: cpfNorm });
    req.session.user = { id, nome, email: String(email).toLowerCase(), cpf: cpf.replace(/\D/g, '') };
    res.redirect(next || '/carrinho');
  } catch (e) {
